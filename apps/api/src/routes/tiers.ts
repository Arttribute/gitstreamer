...
const tiers = new Hono<{ Variables: ContextVariables }>();
...
tiers.post("/project/:id/assign", projectOwnerMiddleware, async (c) => {
  ...
  if (!contributor) {
    ...
    await db.collection<Contributor>("contributors").insertOne({
      projectId: project._id!,
      githubUsername: parsed.githubUsername,
      tier: parsed.tier,
...
tiers.delete("/project/:id/assign/:username", projectOwnerMiddleware, async (c) => {
  ...
  const result = await db.collection<Contributor>("contributors").updateOne(
    {
      projectId: project._id,
