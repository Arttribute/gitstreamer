...
projects.post("/", async (c) => {
  ...
  const existing = await db.collection<Project>("projects").findOne({
...
projects.put("/:id/tiers", projectOwnerMiddleware, async (c) => {
  ...
  if (totalTierShare + tierConfig.treasuryShare !== 100) {
    throw new ValidationError(
      "Total tier shares plus treasury must equal 100%"
    );
  }
...
projects.delete("/:id", projectOwnerMiddleware, async (c) => {
  ...
  await db.collection<Project>("projects").deleteOne({ _id: project._id });
